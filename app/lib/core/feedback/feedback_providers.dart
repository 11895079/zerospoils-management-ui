...
final feedbackServiceProvider =
    FutureProvider<FeedbackService>((ref) async {
...
final hapticEnabledProvider = StateProvider<bool>((ref) {
  return ref.watch(feedbackServiceProvider).maybeWhen(
        data: (service) => service.hapticEnabled,
        orElse: () => true,
      );
});
...
final audioEnabledProvider = StateProvider<bool>((ref) {
  return ref.watch(feedbackServiceProvider).maybeWhen(
        data: (service) => service.audioEnabled,
        orElse: () => true,
      );
});
...
final beepVolumeProvider = StateProvider<double>((ref) {
  return ref.watch(feedbackServiceProvider).maybeWhen(
        data: (service) => service.beepVolume,
        orElse: () => 0.8,
      );
});
