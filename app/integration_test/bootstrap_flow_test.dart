...
void main() {
  ...
  group('Bootstrap integration tests', () {
    ...
    setUpAll(() async {
      final didInitialize =
          await ensureFirebaseInitializedForIntegrationTests();
      if (!didInitialize) {
        return;
      }
    ...
    testWidgets(
      'Full bootstrap flow: Auth → Token → Entitlements → Flags',
      (tester) async {
        ...
        expect(
          idTokenResult,
          isNotNull,
          reason: 'ID token should be available',
        );
      ...
      skip: !firebaseIntegrationConfigAvailable,
    );

    testWidgets(
      'Remote Config initialized after bootstrap',
      (tester) async {
    ...
    testWidgets(
      'Bootstrap gracefully handles Firebase failures',
      (tester) async {
    ...
    testWidgets(
      'Kill-switch: Feature flag disabled via Remote Config',
      (tester) async {
        // Step 1: Bootstrap Firebase
        await FirebaseBootstrapService.initialize();
        ...
        final featureFlagsEnabled = remoteConfig.getBool(
