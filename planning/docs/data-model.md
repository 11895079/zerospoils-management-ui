...
| `quantity` | Number | Yes | Amount | Decimal, min 0.01 |
...
- `item_consumed`: `{"days_until_expiry": 2, "full_consumption": true}`
- `item_wasted`: `{"waste_reason": "expired", "days_past_expiry": 3}`
...
consumed      # Used up (fully or partially)
wasted        # Discarded/thrown away
...
**Money Saved/Wasted:**
```sql
-- Money saved (consumed items)
SELECT SUM(cost) as money_saved
FROM items
WHERE status = 'consumed' 
  AND updated_at >= NOW() - INTERVAL 30 DAY
  AND cost IS NOT NULL;

-- Money wasted (discarded items)
SELECT category, SUM(cost) as money_wasted, COUNT(*) as items_wasted
FROM items
WHERE status = 'wasted'
  AND updated_at >= NOW() - INTERVAL 30 DAY
  AND cost IS NOT NULL
